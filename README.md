# 📄 Markdown 뷰어 (MD → HWPX · 인쇄/PDF)

마크다운(`.md`) 파일을 **끌어다 놓기/파일 선택**으로 업로드하면 깔끔한 화면으로 보여 주고,
**HWPX(한글)** 파일로 내보내거나 **인쇄(PDF로 저장)** 할 수 있는 정적 웹앱입니다.
...
- 🌗 **라이트/다크 테마**, 자동 **목차(TOC)**·**읽기 진행바**·스크롤 위치 추적, **글자 크기 조절(Aa)**
- 📑 **HWPX 내보내기**, 🖨 **인쇄 / PDF 저장**(텍스트 선택 가능), 📥 **원본 .md 다운로드** (상단 **내보내기** 메뉴)
...
- HWPX 변환은 **클라이언트에서** 수행됩니다. 앱이 파일을 외부로 전송하지 않으며, 다운로드/공유는 사용자가 직접 수행하는 별개 동작입니다.
- 클릭재킹 방지를 위해 다른 사이트의 iframe 안에서는 동작하지 않습니다(frame-busting). meta 기반 CSP의 한계로 `frame-ancestors`는 적용되지 않습니다.
- 동봉 라이브러리 버전: marked 12.0.2 · DOMPurify 3.1.6 · highlight.js 11.9.0 · JSZip 3.10.1 (보안 패치는 주기적으로 갱신 권장)
...
- 내보내기는 상단 **내보내기 ▾** 메뉴에서:
  ...
  - **인쇄 / PDF 저장** — 브라우저 인쇄 대화상자에서 *대상: PDF로 저장* 을 선택하면 **텍스트 선택·검색이 가능한** PDF가 만들어집니다.
  - **원본 .md 다운로드** — 업로드한 파일 그대로 저장.
- 본문 **글자 크기**는 상단 **Aa** 버튼으로 4단계 조절할 수 있습니다(설정은 브라우저에 기억됨).
- 아이폰(iOS Safari)에서는 HWPX 등 다운로드 시 **공유 시트**가 떠 *파일 앱에 저장*할 수 있습니다.

### HWPX 변환 범위
표·코드블록·제목·목록·인용·굵게/기울임/취소선/인라인코드/링크(URL 병기)는 반영됩니다.
제목/소제목(H1·H2)에는 **하단 밑줄과 넉넉한 여백**, 본문은 **10.5pt·185% 줄간격·문단 간격**으로 가독성 있게 서식이 적용됩니다.
